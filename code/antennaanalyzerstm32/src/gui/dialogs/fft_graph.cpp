...
void gui_fft_graph::init()
{
    ...
    lv_chart_set_range(m_chart, 0,200);
    ...
    lv_chart_set_point_count(m_chart, 1024);
...
void gui_fft_graph::tick()
{
    ...
    for(int i = 0; i < 1024; i++)
    {
        if(i < 512)
        {
            m_dl1->points[i] = int(g_fft.vv[i].magnitude());
            m_dl2->points[i] = 0;
        }
        else
        {
            m_dl1->points[i] = 0;
            m_dl2->points[i] = int(g_fft.vi[i-512].magnitude());
        }
...
IMPLEMENT_DIALOG(gui_fft_graph);