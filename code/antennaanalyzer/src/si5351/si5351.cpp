...
{
	if(HAL_I2C_IsDeviceReady(&hi2c1, i2c_bus_addr, 10, 0xff) == HAL_OK)
	{
...
uint8_t Si5351::si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
    ...
    if(HAL_I2C_Master_Transmit(&hi2c1, i2c_bus_addr, i2c_buffer, bytes+1, 0xff) != HAL_OK)
    {
        return 4;
    }
    else
...
uint8_t Si5351::si5351_write(uint8_t addr, uint8_t data)
{
    ...
    if(HAL_I2C_Master_Transmit(&hi2c1, i2c_bus_addr, i2c_buffer, 2, 0xff) != HAL_OK)
    {
        return 4;
    }
    else
...
uint8_t Si5351::si5351_read(uint8_t addr)
{
    ...
    if(HAL_I2C_Master_Transmit(&hi2c1, i2c_bus_addr, &addr, 1, 0xff) != HAL_OK)
    {
        return 0;
    }

    if(HAL_I2C_Master_Receive(&hi2c1, i2c_bus_addr, &reg_val, 1, 0xff) != HAL_OK)
    {
        return 0;
    }
